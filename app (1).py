...
# ------------------------------------------
    ...
    .step-banner { background: #586F56; color: white; padding: 12px 20px; border-radius: 8px; font-weight: 600; font-size: 1.1rem; margin-bottom: 15px; margin-top: 25px; }
    [data-testid="stFileUploadDropzone"] { background-color: #ffffff !important; border: 2px dashed #586F56 !important; border-radius: 12px !important; }
    .canteen-invoice-card { background-color: #FFFFFF; border: 1px solid #EAEAEA; border-radius: 24px; padding: 30px; box-shadow: 0 10px 30px rgba(0,0,0,0.02); }
    .invoice-header { display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid #EEEEEE; padding-bottom: 15px; margin-bottom: 20px; }
    .invoice-title { font-size: 1.8rem; font-weight: 700; color: #264653; margin: 0; }
    .invoice-subtitle { font-size: 0.95rem; color: #9A9A9A; margin-top: 5px; }
    .model-badge { background-color: #E2E8F0; color: #4A5568; font-family: monospace; padding: 4px 10px; border-radius: 6px; font-size: 0.75rem; font-weight: bold; }
    .receipt-btn { background-color: #586F56; color: white; padding: 6px 14px; border-radius: 20px; font-size: 0.85rem; font-weight: 600; }
    .food-item-row { display: flex; align-items: center; justify-content: space-between; padding: 15px 0; border-bottom: 1px dashed #F0F0F0; }
    ...
    .food-number-tag { position: absolute; top: -5px; left: -5px; background: #4A4A4A; color: white; font-size: 0.7rem; font-weight: bold; padding: 2px 6px; border-radius: 8px; }
    .food-details { display: flex; flex-direction: column; gap: 4px; }
    .food-title { font-size: 1.05rem; font-weight: 600; color: #333333; }
    ...
    .accuracy-badge { background-color: #EDF2F7; color: #4A5568; font-size: 0.75rem; font-weight: 500; padding: 3px 8px; border-radius: 4px; }
    .food-price { font-size: 1.15rem; font-weight: 700; color: #333333; }
    .total-box { background-color: #FDFBF7; border-radius: 16px; padding: 20px; display: flex; justify-content: space-between; align-items: center; margin-top: 25px; margin-bottom: 25px; }
    .total-label { font-size: 1.1rem; font-weight: bold; color: #8C8275; letter-spacing: 0.5px; }
    .total-price-value { font-size: 2.3rem; font-weight: 800; color: #435241; }
    .payment-title { font-size: 0.85rem; font-weight: 700; color: #A39684; text-align: center; margin-bottom: 15px; letter-spacing: 0.5px; }
    ...
    .pay-active { background-color: #E6ECE6; border: 2px solid #586F56; color: #2F3E2E; }
    .pay-inactive { background-color: #FFFFFF; border: 1px solid #E0E0E0; color: #6E7B8B; }
    .terminal-console { background-color: #1A1A1A; border-radius: 12px; padding: 15px; font-family: monospace; color: #888888; font-size: 0.85rem; max-height: 120px; overflow-y: auto; }
    </style>
    """
    ...
    st.markdown("<h2 style='text-align: center; color: #264653;'>HỆ THỐNG KIỂM TRA & THANH TOÁN KHAY CƠM TỰ ĐỘNG</h2>", unsafe_allow_html=True)
    st.write("---")
    ...
    if active_file is not None:
                ...
                <div class='food-item-left'>
                    ...
                    <div class='food-details'>
                        ...
                        <div class='badge-container'>
                            <span class='category-badge' style='background-color: {bg_color}; color: {text_color};'>{badge_text}</span>
        ...
        with col_right:
            ...
            st.markdown("<br>", unsafe_allow_html=True)
            console_content = "".join([f"<div style='margin-bottom: 4px;'>{log}</div>" for log in ai_console_logs])
